...
确保 `stablepayai-idl/docker-compose.infra.yml` 与 `docker-compose.services.yml` 已启动，或使用线上网关 `https://ai.wenfu.cn`。
...
- `backendBaseUrl` 默认 `https://ai.wenfu.cn`
...
6) 为什么我服务启动了插件还连不上？
- 先确认 `backendBaseUrl` 是否配为 `https://ai.wenfu.cn`（或本地 `28080`），再确认网络连通性。
