...
- ClawHub 安装 slug：`stablepay-plugin`
- 插件 runtime id（manifest）：`stablepay-openclaw-plugin`
...
- 当前 package slug 是 `stablepay-plugin`，runtime id 是 `stablepay-openclaw-plugin`。请在 `plugins.entries` 使用 `stablepay-openclaw-plugin`。
