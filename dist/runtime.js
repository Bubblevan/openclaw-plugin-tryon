...
export class StablePayRuntime {
    ...
    async createLocalWallet(params) {
        const state = await this.loadState();
    ...
    async configurePaymentLimits(params) {
    ...
    async buildPaymentPolicy(params) {
        ...
        const manifest = {
            ...
            runtime_driver: state.wallet.runtimeDriver,
            note: "This manifest is OWS-ready for the StablePay client flow. Registering it into an OWS vault is deferred until the target runtime supports it in this environment.",
        };
     ...
     */
    async signSolanaTransactionMessageHex(messageHex) {
        const state = await this.requireWalletState();
        const cfg = this.cfg;
        ...
        if (state.runtimeDriver === "ows-sdk") {
            ...
            if (!ows)
                throw new Error("OWS SDK runtime is not available in this environment");
            const raw = ows.signMessage(state.walletName, "solana", clean, this.getOptionalEnv(cfg.owsPassphraseEnv), "hex", 0, cfg.owsVaultPath || undefined).signature;
    ...
    async createWithOwsSdk(walletName) {
        ...
        const wallet = ows.createWallet(walletName, this.getOptionalEnv(this.cfg.owsPassphraseEnv), undefined, this.cfg.owsVaultPath || undefined);
    ...
    createOwsCliLinkedWallet(walletName, publicKeyBase58, driver) {
        return {
            walletId: walletName,
            walletName,
    ...
    createOwsRestLinkedWallet(walletName, publicKeyBase58, owsWalletUUID) {
        return {
    ...
    async signWithOwsRestHex(walletId, messageHex) {
        const cfg = this.cfg;
    ...
    async signWithOwsRest(chainId, walletId, message) {
        const base = this.cfg.owsRestBaseUrl.replace(/\/+$/, "");
        const signPath = this.cfg.owsRestSignPath.startsWith("/")
            ? this.cfg.owsRestSignPath
            : `/${this.cfg.owsRestSignPath}`;
    ...
    async detectAvailability(preferred) {
        ...
        if (ows) {
            ...
            notes.push("OWS Node SDK could not be loaded in this environment. On the current Windows machine, the official package does not ship a win32 native binding yet.");
        }
        ...
        else if (this.cfg.owsRestBaseUrl &&
        ...
        if (activeDriver === "ows-rest") {
            notes.push("Using HTTP signMessage against owsRestBaseUrl. Align request/response with your OWS access-layer implementation.");
        }
        return {
            requestedDriver,
            activeDriver,
    ...
    requireMasterKey() {
        ...
        if (!key) {
            throw new Error(`Missing ${this.cfg.localStateKeyEnv}. Set this environment variable before using the local wallet runtime.`);
        }
        return key;
    }
    getOptionalEnv(name) {
...
function signWithOwsCli(walletName, chain, message) {
    const result = spawnSync("ows", ["sign", "message", "--wallet", walletName, "--chain", chain, "--message", message, "--json"], { encoding: "utf8", maxBuffer: 16 * 1024 * 1024, env: process.env });
