 ...
 */
export function parseSignatureHexFromOwsJson(jsonText: string): string {
  ...
  const keys = ["signature", "sig"] as const;
  for (const k of keys) {
    const v = parsed[k];
 ...
 */
export function signMessageHexWithOwsCli(walletName: string, chain: string, messageHex: string): string {
  ...
  const result = spawnSync(
    "ows",
    ["sign", "message", "--wallet", walletName, "--chain", chain, "--encoding", "hex", "--message", cleanHex, "--json"],
 ...
 */
export function signSolanaMessageHexWithOwsCli(walletName: string, messageHex: string): string {
 ...
 */
export function parseSignedTxHexFromOwsJson(jsonText: string): string {
  let parsed: Record<string, unknown>;
  try {
    parsed = JSON.parse(jsonText) as Record<string, unknown>;
  } catch {
  ...
  const keys = ["signed_tx", "signedTx", "tx", "transaction", "raw_tx", "rawTransaction"] as const;
  for (const k of keys) {
    const v = parsed[k];
    if (typeof v === "string" && v.trim()) {
      let h = v.trim();
 ...
 */
export function signTxWithOwsCli(walletName: string, chain: string, txHex: string): string {
  const result = spawnSync(
    "ows",
    ["sign", "tx", "--wallet", walletName, "--chain", chain, "--tx", txHex, "--json"],
    { encoding: "utf8", maxBuffer: 32 * 1024 * 1024, env: process.env },
  );
  const errText = (result.stderr || result.stdout || result.error?.message || "").trim();
  if (result.status !== 0) {
