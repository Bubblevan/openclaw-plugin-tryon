...
export type PaySettlementResult = {
  ...
  currency: "USDC" | "USDT";
  amount_minor: number;
  signed_tx_hash_sha256: string;
  order_id: string;
  gateway: { timestamp: string; nonce: string; canonical: string };
  business: { message_core: string; timestamp: number; nonce: string };
  compact_body: string;
};

function solanaPubkeyFromSkillDid(skillDid: string): string {
...
function resolveFeePayer(cfg: Required<PluginConfig>): string {
  const fromEnv = process.env.STABLEPAY_FEE_PAYER_SOL?.trim();
  if (fromEnv) return fromEnv;
  const fromCfg = cfg.feePayerSolanaAddress?.trim();
  if (fromCfg) return fromCfg;
  throw new Error(
    "Missing fee payer (hotwallet) Solana address. Set plugin config feePayerSolanaAddress or env STABLEPAY_FEE_PAYER_SOL to match blockchain-adapter hotwallet.json.",
  );
}

export function toMinorUnitsInt(amount: string): number {
...
function currencyEnum(currency: "USDC" | "USDT"): number {
...
export async function settlePaymentViaGateway(input: {
  ...
  paymentLimits: PaymentConfigSnapshot;
  /** When true, proceed even if price > autoPurchaseThresholdUsdc (user confirmed in chat). */
  confirmOverThreshold?: boolean;
}): Promise<
  | { ok: true; result: PaySettlementResult }
  | { ok: false; status: "policy_denied" | "manual_confirmation_required"; detail: string; requirement: PaymentRequirement }
> {
