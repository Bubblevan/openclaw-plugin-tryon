...
export class StablePayRuntime {
  ...
  private async loadState(): Promise<LocalPluginState> {
    try {
      ...
      const hint =
        "If the state file already exists, the master key must match (set the same STABLEPAY_PLUGIN_MASTER_KEY / localStateKeyEnv as when it was created).";
      throw new Error(
        `Failed to load local plugin state (${this.cfg.localStatePath}): ${error instanceof Error ? error.message : String(error)}. ${hint}`,
      );
    }
  }
